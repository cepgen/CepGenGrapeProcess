...
*     original FFV0 (inlined)
*-----------------------------------------------------------------------
      SUBROUTINE FFVMM(LIND,II,IO,AAM,AM,AL,AR,CC,C,Q1,Q2,P1,P2,Q,AALL)
...
C-
        ...
        IF (AAM.LE.0.0D0) THEN
               RR    = 0.0D0
        ENDIF
*       IF (AM.LE.0.0D0 .OR. AAM.LE.0.0D0) THEN
*           print *,'AM = ',AM,'AAM = ',AAM
*       ENDIF
C
           J1    = (5-II)/2
...
C- Q1,P1
     ...
     .                 +(Q(4)-Q(1))*(Q1R1Z*P1R2Y-Q1R1Y*P1R2Z)
           ENDIF
      IF( AM.GT.0.0D0 .AND. AAM.GT.0.0D0 ) THEN
C- Q2,P2
           IF( LQ2.AND.LP2 ) THEN
...
C---
     ...
     &                 +(AL*R*RR*C(1)*CC(1)*DCMPLX(ALL22R,-ALL22I))
      ...
      ELSE
           AALL(LIND,J1,K2)= 0.0D0
           AALL(LIND,J2,K1)= 0.0D0
      ENDIF
      IF( AM .GT. 0.0D0 ) THEN
           AALL(LIND,J1,K2)= AALL(LIND,J1,K2)
     &                 -(AL*R*C(1)*DCMPLX(ALL12R,-ALL12I))
           AALL(LIND,J2,K1)= AALL(LIND,J2,K1)
