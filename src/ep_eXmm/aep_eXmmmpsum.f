*     File ep_eXmm/aep_eXmmmpsum.f : Sat Mar 18 19:45:45 2000
*
...
*       sum over color bases
     ...
     &                * www(mu,mu, abs(vnep_eXmm10),amass1(3),
     &                    pfep_eXmm2,pfep_eXmm11, kcharg(1)/3,amass1(1))
          anc = anc + anc_www

          if ((mu.GT.1).AND.(mu.LT.4)) then
             do nu=1,mu-1
               anc_www = 2.D0*dble( agcr(ind,mu)*conjg(agcr(ind,nu)) )
     &                    *www(mu,nu, abs(vnep_eXmm10),amass1(3),
     &                       pfep_eXmm2,pfep_eXmm11,
     &                       kcharg(1)/3,amass1(1))
               anc = anc + anc_www
             enddo
           elseif (mu.EQ.4) then
